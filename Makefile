...
# Filters our variables that we don't want to send to the .env file, extend as needed
FILTER_VARS := $(.VARIABLES)
FILTER_VARS += TEMP
################################################################################
# Workcell Configuration: Adjust these parameters to meet your needs
# You can override these at run time by running `make <target> <VARIABLE>="..."`

# Workcell and Project Files/Directories
...
default: start run # Builds, registers diaspora if necessary, starts, and runs APP_NAME
...
run: start # Runs APP_COMMAND in a container named APP_NAME
...
# except for anything in FILTER_VARS and FILTER_VARS itself
	@$(foreach v,\
